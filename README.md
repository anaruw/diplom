...
6. Завершить работу сервера Allure (сочетанием клавиш *Ctrl+C*, или иным способом, указанном в подсказке)
7. Завершить работу *aqa-shop.jar*. Варианты:
...
9. ```git log --oneline``` Узнать, в какой ветке<sup>*</sup> проекта находимся(указатель *HEAD*)
10. ```git switch postgresql``` Переключиться на ветку<sup>*</sup> *postgresql*(если в п.6 мы находились в этой ветке, заменить *postgresql* на *main*)
11. Повторить п.п. 3-7 для второй ветки проекта
12. ```docker compose down``` Завершить работу контейнеров
